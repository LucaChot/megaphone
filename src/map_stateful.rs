...
use stateful::{StateProtocol, Notificator};
...
pub struct MapStateStream<S, V, D, W, M> where
    ...
    M: ExchangeData,
{
    ...
    /// A handle to the shared mapping
    pub config: Rc<RefCell<ControlSet<S::Timestamp>>>,
    /// The probe `stateful` uses to determine completion.
    pub feedback: FeedbackHandle<S, ()>,
    _phantom: PhantomData<(*const W)>,
}

impl<S, V, D, W, M> MapStateStream<S, V, D, W, M>
    where
        S: Scope, // The containing scope
        S::Timestamp: TotalOrder,
        V: ExchangeData, // Input data
        ...
        M: ExchangeData,
{
    /// Creates a new MapStateStream
    pub fn new(stream: Stream<S, (usize, Key, V)>, state_stream: Stream<S, (usize, StateProtocol<S::Timestamp, W, M>)>, state: Rc<RefCell<State<S::Timestamp, D, M>>>, config : Rc<RefCell<ControlSet<S::Timestamp>>>, feedback: FeedbackHandle<S, ()>) -> Self {
        MapStateStream {
            stream,
            state_stream,
            state,
...
pub trait MapStateful<S: Scope, V: ExchangeData> {
    ...
    /// * `B`: Key function
    fn map_stateful<W, D, B, M>(&self, key: B, control: &Stream<S, Control>) -> MapStateStream<S, V, D, W, M>
        where
...
impl<S: Scope, V: ExchangeData> MapStateful<S, V> for Stream<S, V> {
            ...
            M: ExchangeData,
    {
        ...
        let active_configuration = Rc::new(RefCell::new(ControlSet { 
                sequence: 0,
                frontier: Antichain::from_elem(Default::default()),
                map,
            }));
        let outer_active_configuration = active_configuration.clone();
...
//        let probe2 = probe1.clone();
            ...
            move |frontiers| {
                ...
                control_notificator.for_each(&[&frontiers[1]], |cap, time, _not| {
                    ...
                    if let Some(builder) = pending_configuration_data.remove(&time) {
                        ...
                        let config = builder.build(pending_configurations.last().map_or(&active_configuration.borrow(), |pending| &pending.1));
                        ...
                        if let Some(config) = pending_configurations.first() {
                            debug_assert!(active_configuration.borrow().frontier.dominates(&config.1.frontier));
                        }
                    }
                });
                ...
                if pending_configurations.get(0).is_some() {
                    if pending_configurations[0].1.frontier.elements().iter().all(|t| !frontiers[2].less_than(t)) {
                        ...
                        {   // Scoped to let `old_map` and `new_map` borrows drop.
                            let current = active_configuration.borrow();
                            let old_map = current.map();
                        ...
                        let mut mut_active_config = active_configuration.borrow_mut();
                        *mut_active_config = to_install;
                    }
                }
                ...
                data_notificator.for_each(&[&frontiers[0], &frontiers[1]], |cap, time, _not| {
                    ...
                    if let Some(vec) = data_stash.remove(&time) {
                        ...
                        let mut future_configuration = &*active_configuration.borrow();
                        if let Some(config) =
                            pending_configurations
                                ...
                                future_configuration = config;
                        };
                        let map = future_configuration.map();
        ...
        MapStateStream::new(stream, state, states, outer_active_configuration, feedback_handle)
    }
}

#[cfg(feature = "fake_stateful")]
impl<S: Scope, V: ExchangeData> Stateful<S, V> for Stream<S, V> {
    fn stateful<W, D, B, M>(&self, key: B, _control: &Stream<S, Control>) -> MapStateStream<S, V, D, W, M>
        where
            S::Timestamp : Hash+Eq+TotalOrder,
        // State format on the wire
            W: ExchangeData,
            ...
            M: ExchangeData,
    {
        ...
        MapStateStream::new(stream, state_stream, states, feedback_handle)
    }
}
